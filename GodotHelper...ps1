...
.NOTES
    ...
    Függőségek:    GodotHelper.dll (GAC)
                   EPPlus.dll (GAC)
                   CvsHelper.dll (GAC)
                   MySql.Data.dll (GAC)
#>
function GodotHelper { Get-Help GodotHelper }

Set-Alias -Name Godot-Helper -Value GodotHelper

<#
.SYNOPSIS
    Betölti a GodotHelper.dll-t a PowerShell környezetbe.
.DESCRIPTION
    Betölti a GodotHelper.dll-t a PowerShell környezetbe.
    Meghívása a szkript futtatásakor automatikusan megtörténik, nem szükséges újból futtatni.
.PARAMETER BaseDir
    A GodotHelper.dll elérési útvonala
    Alapértelmezett érték: aktuális könyvtár
.EXAMPLE
    . .\GodotHelper.ps1
##
function Import-GodotHelper
{
  param ([string] $BaseDir = (Get-Location).Path)
  $path = [System.IO.Path]::GetFullPath([System.IO.Path]::Combine($baseDir, "GodotHelper.dll"))
  if (!(Test-Path $path -PathType Leaf))
  {
    throw "Nem találom a GodotHelper.dll fájl!"
  }
  [Reflection.Assembly]::LoadFile($path) | out-null
}

# GodotHelper.dll betöltése
if (![string]::IsNullOrWhiteSpace($args[0]) -and (Test-Path $args[0] -PathType Container))
{
  Import-GodotHelper $args[0]
} else {
  Import-GodotHelper
}
#>
...
try
{
    # GodotHelper betöltése
    [Reflection.Assembly]::Load("GodotHelper, Version=3.0.0.0, Culture=neutral, PublicKeyToken=abd5f8293fa96958, processorArchitecture=MSIL") | out-null
}
catch
{
    Write-Host "A 3.0-ás verziójú GodotHelper.dll nincs telepítve a rendszeren vagy nincs engedélyezve a PowerShell számára a .NET 4.0-ás szerelvények használata. A telepítéshez indítsd el az install_epplus.cmd és az install_godothelper.cmd fájlokat adminisztrátori parancssorból. A .NET 4.0 engedélyezéséhez indítsd el az Enable .NET 4.0 for PowerShell.cmd nevű fájlt szintén adminisztrátori parancssorból."
}
...
function Start-Log
{
  ...
  [Godot.Log]::Start($FileName, $Clear, $Console)
}
...
function Write-Log
{
  param ([parameter(Mandatory=$true,HelpMessage="A naplózandó üzenet")] [string] $Message,
         [string] $Source = "",
         [Godot.LogLevel] $Level = [Godot.LogLevel]::Normal)
  [Godot.Log]::Write($Message, $Source, $Level)
}
...
.DESCRIPTION
    ...
    A GodotHelper cmdlet-jei önállóan is képesek naplózni a tevékenységüket, nem szükséges explicit naplózni az eljáráshívásokat.
.PARAMETER Level
...
{
  param ([parameter(Mandatory=$true, HelpMessage="Az új naplózási szint")] [Godot.LogLevel] $Level)
  [Godot.Log]::Level = $Level
}
...
{
  [Godot.Log]::Encoding.EncodingName
}
...
function Set-Log-Encoding
{
  ...
  [Godot.Log]::Encoding = [System.Text.Encoding]::GetEncoding($Encoding)
}
...
.OUTPUTS
    Godot.IDb. Adatbázis-kapcsolat objektum
.LINK
    Db-Connection2
    Execute-Query
    Execute-Query-To-Csv
...
function Db-Connect
{
  ...
  if ($SqlServer)
  {
    $db = New-Object Godot.Sql($ConnStr)
  }
  elseif ($MySql)
  {
    $db = New-Object Godot.MySql($ConnStr)
  }
  else
  {
    $db = New-Object Godot.Oracle($ConnStr)
  }
...
function Db-Connect2
{
  ...
  if ($SqlServer)
  {
    $db = New-Object Godot.Sql($DataSource, $UserId, $Password)
  }
  elseif ($MySql)
  {
    $db = New-Object Godot.MySql($DataSource, $UserId, $Password, $Server, $Port)
  }
  else
  {
    $db = New-Object Godot.Oracle($DataSource, $UserId, $Password)
  }
  Set-Variable -Name _lastDbConn -Value $db -Scope Script
  return $db
}
...
.OUTPUTS
    Godot.Results. A lekérdezés eredményeit tartalmazó objektum
.LINK
    Db-Connection
    Db-Connection2
    Execute-Query-To-Csv
...
function Dispose-Db
{
  param ([Godot.IDb] $DbConn = $_lastDbConn)
  if ($DbConn)
  {
...
.DESCRIPTION
    Felszabadítja a Godot.Results típusú eredmény objektum által lefoglalt memóriát
.PARAMETER Results
    A felszabadítandó Godot.Results típusú eredmény objektum
.LINK
    Db-Connection
    Db-Connection2
    Execute-Query
...
{
  param([parameter(Mandatory=$true,HelpMessage="A felszabadítandó Godot.Results típusú objektum")] [Godot.Results] $Results)
...
.OUTPUTS
    Godot.Excel. Excel-munkafüzet objektum
.LINK
    Add-Worksheet
    Add-Comment
    Set-Worksheet-Styles
...
function Create-Workbook
{
  ...
  Consolidate-WorkingDir
  $workbook = New-Object Godot.Excel($Template)
...
{
  param([parameter(Mandatory=$true,HelpMessage="A beszúrandó eredményhalmaz")] [Godot.Results] $Results,
        [parameter(Mandatory=$true,HelpMessage="A beszúrandó munkalap neve")] [string] $Name,
        [Godot.Excel] $Workbook = $_lastWorkbook)
...
function Set-Worksheet-Styles
{
  param([bool] $AutoFilter,
        ...
        [string] $DateFormat,
        [switch] $Reset)
  if ($Reset) { [Godot.Excel]::ResetStyles() }
  if ($PSBoundParameters.ContainsKey("AutoFilter")) { [Godot.Excel]::AutoFilter = $AutoFilter }
  if ($PSBoundParameters.ContainsKey("AutoFit")) { [Godot.Excel]::AutoFit = $AutoFit }
  if ($PSBoundParameters.ContainsKey("Header")) { [Godot.Excel]::Header = $Header }
  if ($PSBoundParameters.ContainsKey("HeaderBold")) { [Godot.Excel]::HeaderBold = $HeaderBold }
  if ($PSBoundParameters.ContainsKey("HeaderTextRotation")) { [Godot.Excel]::HeaderTextRotation = $HeaderTextRotation }
  if ($PSBoundParameters.ContainsKey("FreezePanes")) { [Godot.Excel]::FreezePanes = $FreezePanes }
  if ($PSBoundParameters.ContainsKey("HeaderStartRow")) { [Godot.Excel]::HeaderStartRow = $HeaderStartRow }
  if ($PSBoundParameters.ContainsKey("HeaderStartCol")) { [Godot.Excel]::HeaderStartCol = $HeaderStartCol }
  if ($PSBoundParameters.ContainsKey("DataStartRow")) { [Godot.Excel]::DataStartRow = $DataStartRow }
  if ($PSBoundParameters.ContainsKey("DataStartCol")) { [Godot.Excel]::DataStartCol = $DataStartCol }
  if ($PSBoundParameters.ContainsKey("DefaultColWidth")) { [Godot.Excel]::DataStartRow = $DefaultColWidth }
  if ($PSBoundParameters.ContainsKey("DefaultRowHeight")) { [Godot.Excel]::DataStartCol = $DefaultRowHeight }
  if ($PSBoundParameters.ContainsKey("MinColWidth")) { [Godot.Excel]::MinColWidth = $MinColWidth }
  if ($PSBoundParameters.ContainsKey("MaxColWidth")) { [Godot.Excel]::MaxColWidth = $MaxColWidth }
  if ($PSBoundParameters.ContainsKey("DateFormat")) { [Godot.Excel]::DateFormat = $DateFormat }
}
...
function Send-Mail
{
  ...
  if ($Attachments) {
    Consolidate-WorkingDir
    [Godot.Mail]::Send($From, $To, $Cc, $Bcc, $Subject, $Body, $Priority, $Html, $SmtpHost, $SmtpPort, $Attachments)
  } else {
    [Godot.Mail]::Send($From, $To, $Cc, $Bcc, $Subject, $Body, $Priority, $Html, $SmtpHost, $SmtpPort)
  }
}
...
{
  [Godot.Mail]::BodyEncoding.EncodingName
}
...
function Set-Mail-Body-Encoding
{
  ...
  [Godot.Mail]::BodyEncoding = [System.Text.Encoding]::GetEncoding($Encoding)
}
...
{
  [Godot.Mail]::SubjectEncoding.EncodingName
}
...
function Set-Mail-Subject-Encoding
{
  ...
  [Godot.Mail]::SubjectEncoding = [System.Text.Encoding]::GetEncoding($Encoding)
}
...
function Xlsx-To-Csv
{
  param ([parameter(Mandatory=$true,HelpMessage="Bemeneti fájlnév")] [string] $InputFile,
         ...
         [HashTable] $Transform)
  Consolidate-WorkingDir
  [Godot.Excel]::ConvertToCsv($InputFile, $OutputFile, $WorksheetId, $WorksheetName, $Delimiter, $Quote, [System.Text.Encoding]::GetEncoding($Encoding), $Password, $Append, $Transform)
}
...
{
  param ([parameter(Mandatory=$true,HelpMessage="Az elmentendő eredményhalmaz")] [Godot.Results] $Results,
         [parameter(Mandatory=$true,HelpMessage="Kimeneti fájlnév")] [string] $OutputFile,
         [string] [Alias('Separator')] $Delimiter = ";",
         [char]   $Quote = '"',
         [string] $Encoding = "UTF-8",
...
{
  param([parameter(Mandatory=$true,HelpMessage="Az exportálandó eredményhalmaz")] [Godot.Results] $Results,
         [string] $TableStyle,
         [string] $TrStyle,
         [string] $TdStyle,
         [string] $ThStyle,
